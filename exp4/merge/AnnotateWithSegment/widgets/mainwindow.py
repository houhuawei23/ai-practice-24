...
from widgets.converter_dialog import ConverterDialog
from widgets.video_to_frames_dialog import Video2FramesDialog
from widgets.auto_segment_dialog import AutoSegmentDialog
from widgets.model_manager_dialog import ModelManagerDialog
from widgets.annos_validator_dialog import AnnosValidatorDialog
...
import functools
import imgviz
from segment_any.segment_any import SegAny, SegAnyVideo
...
class SegAnyThread(QThread):
    ...
    def sam_encoder(self, image):
        ...
        with torch.inference_mode(), torch.autocast(
            self.mainwindow.segany.device, dtype=self.mainwindow.segany.model_dtype
        ):
            ...
            if "sam2" in self.mainwindow.segany.model_type:
                ...
                input_image = (
                    self.mainwindow.segany.predictor_with_point_prompt._transforms(
                        image
                    )
                )
                input_image = input_image[None, ...].to(
                    self.mainwindow.segany.predictor_with_point_prompt.device
                )
                backbone_out = self.mainwindow.segany.predictor_with_point_prompt.model.forward_image(
                    input_image
                )
                _, vision_feats, _, _ = (
                    self.mainwindow.segany.predictor_with_point_prompt.model._prepare_backbone_features(
                        backbone_out
                    )
                )
                if (
                    self.mainwindow.segany.predictor_with_point_prompt.model.directly_add_no_mem_embed
                ):
                    vision_feats[-1] = (
                        vision_feats[-1]
                        + self.mainwindow.segany.predictor_with_point_prompt.model.no_mem_embed
                    )
                feats = [
                ...
                input_image = self.mainwindow.segany.predictor_with_point_prompt.transform.apply_image(
                    image
                )
                ...
                input_image = (
                    self.mainwindow.segany.predictor_with_point_prompt.model.preprocess(
                        input_image_torch
                    )
                )
                features = self.mainwindow.segany.predictor_with_point_prompt.model.image_encoder(
                    input_image
                )
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def SeganyEnabled(self):
        ...
        if (
            ...
            if self.segany.model_source == "sam_hq":
                ...
                self.segany.predictor_with_point_prompt.interm_features = (
                    interm_features
                )
    ...
    def init_ui(self):
        ...
        # 新增手动/自动 选择group
        self.next_group_shortcut.activated.connect(
            self.annos_dock_widget.go_to_next_group
        )
        self.prev_group_shortcut.activated.connect(
            self.annos_dock_widget.go_to_prev_group
        )

        self.scene = AnnotationScene(mainwindow=self)
        self.category_choice_widget = CategoryChoiceDialog(
            self, mainwindow=self, scene=self.scene
        )
        self.category_edit_widget = CategoryEditDialog(self, self, self.scene)

        # 批量点修改 (issue 160) 快捷键
        self.polygon_repaint_shortcut = QtWidgets.QShortcut(
            QtGui.QKeySequence("R"), self
        )
        self.polygon_repaint_shortcut.activated.connect(
            self.scene.change_mode_to_repaint
        )

        # 新增图片保存功能，快捷键。P保存场景(只图片区域)，Ctrl+P保存整个窗口
        self.scene_shot_shortcut = QtWidgets.QShortcut(QtGui.QKeySequence("P"), self)
        self.scene_shot_shortcut.activated.connect(
            functools.partial(self.screen_shot, "scene")
        )
        self.window_shot_shortcut = QtWidgets.QShortcut(
            QtGui.QKeySequence("Ctrl+P"), self
        )
        self.window_shot_shortcut.activated.connect(
        ...
        # self.testBar.addWidget(self.mask_aplha)
        self.mask_aplha.hide()
        self.mask_aplha.setVisible(False)
        ...
        # self.testBar.addWidget(self.vertex_size)
        self.vertex_size.hide()
        self.vertex_size.setVisible(False)
        ...
        # self.testBar.addWidget(self.image_saturation)
        self.image_saturation.hide()
        self.image_saturation.setVisible(False)
        ...
        # self.testBar.addWidget(self.show_prompt)
        self.show_prompt.hide()
        self.show_prompt.setVisible(False)
        ...
        # self.testBar.addWidget(self.show_edge)
        self.show_edge.hide()
        self.show_edge.setVisible(False)
        ...
        # self.testBar.addWidget(self.use_polydp)
        self.use_polydp.hide()
        self.use_polydp.setVisible(False)
    ...
    def update_menuSAM(self):
        ...
        model_names = sorted(
            [
                pth
                for pth in os.listdir(CHECKPOINT_PATH)
                if pth.endswith(".pth") or pth.endswith(".pt")
            ]
        )
        self.pths_actions = {}
        for model_name in model_names:
            ...
            action.triggered.connect(
                functools.partial(self.init_segment_anything, model_name)
            )
    ...
    def open_dir(self):
        ...
        if os.path.exists(os.path.join(dir, "labels.yaml")):
            # load setting yaml
            self.config_file = os.path.join(dir, "labels.yaml")
            self.reload_cfg()
    ...
    def save(self):
        ...
        self.save_cfg(os.path.join(self.label_root, "labels.yaml"))
        ...
        if index:
            if not index.isdigit():
                if index in self.files_list:
                    ...
                    QtWidgets.QMessageBox.warning(
                        self, "Warning", "Don`t exist image named: {}".format(index)
                    )
    ...
    def change_bit_map_to_semantic(self):
        ...
        for polygon in self.polygons:
            ...
            polygon.change_color(
                QtGui.QColor(self.category_color_dict.get(polygon.category, "#000000"))
            )
    ...
    def init_connect(self):
        ...
        self.actionContour_Max_only.triggered.connect(
            functools.partial(self.change_contour_mode, "max_only")
        )
        self.actionContour_External.triggered.connect(
            functools.partial(self.change_contour_mode, "external")
        )
        self.actionContour_All.triggered.connect(
            functools.partial(self.change_contour_mode, "all")
        )
