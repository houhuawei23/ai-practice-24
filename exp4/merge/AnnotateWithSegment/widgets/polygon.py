...
class Polygon(QtWidgets.QGraphicsPolygonItem):
    def __init__(self):
        ...
        self.rxmin, self.rxmax, self.rymin, self.rymax = (
            0,
            0,
            0,
            0,
        )  # 用于绘画完成后，记录多边形的各边界，此处与points对应
    ...
    def redraw(self):
        ...
        self.rxmin, self.rymin, self.rxmax, self.rymax = (
            min(xs),
            min(ys),
            max(xs),
            max(ys),
    ...
    def set_drawed(
        self, category, group, iscrowd, note, color: QtGui.QColor, layer=None
    ):
